...
\begin{description}[style=nextline, labelindent=2em, leftmargin=4em]
  \item [\PVerb{\cite{amaral15}}] \cite{amaral15}
  \item [\PVerb{\textcite{bosi08}}] \textcite{bosi08}
  \item [\PVerb{\apud{assis08}{bosi08}}] \apud{assis08}{bosi08}
  \item [\PVerb{\cites{moretti09}{mann09}{amaral15}}]
    \cites{moretti09}{mann09}{amaral15}
  \item [\PVerb{\textcites{moretti09}{mann09}{amaral15}}]
...
\begin{description}[style=nextline, labelindent=2em, leftmargin=4em]
  \item [\PVerb{\cites{mann09}{moretti09:1, moretti09}}]
    \cites{mann09}{moretti09:1, moretti09}
  \item [\PVerb{\apud[p.~12]{assis08}[p.~200]{bosi08}}]
    \apud[p.~12]{assis08}[p.~200]{bosi08}
  \item [\PVerb{\textapud[p.~200]{assis08}[p.~12]{bosi08}}]
    \textapud[p.~200]{assis08}[p.~12]{bosi08}
  \item [\PVerb{\apud[batman][]{bosi08}}] \apud[batman][]{bosi08}
  \item [\PVerb{Assis \cite[apud][p.~200]{bosi08}}] Assis
    \cite[apud][p.~200]{bosi08}
...
Exceto pelo comando \PVerb{\textcite*}, que imprime o nome completo do autor,
    as versões com asterisco dos comandos acima imprimem o mesmo texto sem os
    parênteses:

\begin{description}[style=nextline, labelindent=2em, leftmargin=4em, parsep=4pt]
  \item [\PVerb{\textcite*{bosi08}}] \textcite*{bosi08}
...
\begin{description}[style=nextline, parsep=4pt]
    ...
    \item [\PVerb{Segundo o autor\apud{assis08}{bosi08}}] Segundo o autor\apud{assis08}{bosi08}
\end{description}
...
\begin{sloppypar}
    \item Não consegui entender por que, mas os exemplos do
      \texttt{abntex2cite} usam \verb"\'\i" para o caractere ``í'', enquanto
      o normal me parece ser \verb"\'i" (para outras letras com acento agudo
      o \texttt{abntex2cite} usa o formato normal). Isso pode causar alguns
    ...
    \item Os campos de datas (\texttt{date}, \texttt{year}, \texttt{month}
      etc.) devem seguir o formato usado pelo \texttt{biblatex}
      (\texttt{yyyy-mm-dd}). Cf. seção 2.2.1 do manual.
    \item O \texttt{biblatex} diferencia os campos \texttt{pages} (e.g. ``p.
      12-18'') e \texttt{pagetotal} (e.g. ``347 p.'').
    \item Quando o primeiro campo impresso é o título, a primeira palavra
      é automaticamente impressa em maiúsculas. Para que mais de uma palavra
      seja impressa em maiúsculas, coloque-as entre chaves ou separadas por um
      \emph{no breaking space} (\verb"~").
    \item Quando o primeiro campo impresso é a organização, todo o campo
      é impresso em maiúsculas. Para que a capitalização de uma palavra não
    ...
    \item Pode-se incluir no campo \texttt{options} as opções \texttt{nosl}
      ...
      mostrar ``[s.n.]'') ou \texttt{noslsn} (para não mostrar nenhuma das
    ...
    \item Quando o nome do autor, editor ou organização for muito grande para
      ...
      pelo \texttt{abntex\-2\-cite}, é automaticamente convertido para
      \texttt{shortauthor}.
